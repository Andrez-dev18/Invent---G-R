...
public class EntradaJpaController implements Serializable {
    ...
    public void create(Entrada entrada) {
        ...
        try {
            ...
            Producto unProducto = entrada.getUnProducto();
            if (unProducto != null) {
                unProducto = em.getReference(unProducto.getClass(), unProducto.getId());
                entrada.setUnProducto(unProducto);
            }
            em.persist(entrada);
            if (unProducto != null) {
                unProducto.getEntradas().add(entrada);
    ...
    public void destroy(int id) throws NonexistentEntityException {
        ...
        try {
            ...
            Producto unProducto = entrada.getUnProducto();
            if (unProducto != null) {
                unProducto.getEntradas().remove(entrada);
                unProducto = em.merge(unProducto);
            }
            em.remove(entrada);
            em.getTransaction().commit();
