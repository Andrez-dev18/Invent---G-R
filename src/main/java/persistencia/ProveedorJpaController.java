    ...
    public void create(Proveedor proveedor) {
        ...
        try {
            ...
            for (Producto productosProducto : proveedor.getProductos()) {
                Proveedor oldUnProveedorOfProductosProducto = productosProducto.getUnProveedor();
                productosProducto.setUnProveedor(proveedor);
                productosProducto = em.merge(productosProducto);
                if (oldUnProveedorOfProductosProducto != null) {
                    oldUnProveedorOfProductosProducto.getProductos().remove(productosProducto);
                    oldUnProveedorOfProductosProducto = em.merge(oldUnProveedorOfProductosProducto);
                }
            }
        ...
        try {
            ...
            for (Producto productosOldProducto : productosOld) {
                if (!productosNew.contains(productosOldProducto)) {
                    productosOldProducto.setUnProveedor(null);
            ...
            for (Producto productosNewProducto : productosNew) {
                if (!productosOld.contains(productosNewProducto)) {
                    Proveedor oldUnProveedorOfProductosNewProducto = productosNewProducto.getUnProveedor();
                    productosNewProducto.setUnProveedor(proveedor);
                    productosNewProducto = em.merge(productosNewProducto);
                    if (oldUnProveedorOfProductosNewProducto != null && !oldUnProveedorOfProductosNewProducto.equals(proveedor)) {
                        oldUnProveedorOfProductosNewProducto.getProductos().remove(productosNewProducto);
                        oldUnProveedorOfProductosNewProducto = em.merge(oldUnProveedorOfProductosNewProducto);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(int id) throws NonexistentEntityException {
        ...
        try {
            ...
            for (Producto productosProducto : productos) {
                productosProducto.setUnProveedor(null);
                productosProducto = em.merge(productosProducto);
            }
            em.remove(proveedor);
            em.getTransaction().commit();
